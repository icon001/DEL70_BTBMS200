...
//  TSystemType = (stDoor,stAlarm,stGrade);
  ...
  TDeviceAlarmEvent = procedure(Sender:TObject;NodeNo:integer;ECUID:string;aArmAreaNo:integer;aAlaramState:TAlarmEventState) of object;
...
//  ATTENDTYPE : String; //근태사용타입(BUTTON/UPDATE/READERNO(1/2))
  ...
  G_bColseModeView : Boolean = True;  //폐쇄모드 사용 유무
  ...
  //******************카드 고정길이 사용 환경
  G_bCardFixedUse : Boolean=True;   //카드고정 사용 유무
  G_stCardFixedFillChar : string='*'; //카드고정시 채움문자
  G_nCardFixedPosition : integer=1; //채움문자위치 0:앞,1:뒤
  G_nCardFixedLength : integer=16;   //카드고정길이
  G_nCardMaxCount : integer;      //카드 최대 갯수
...
begin
  ...
  LogSave(ExeFolder + '\..\log\Del'+ FormatDateTIme('yyyymmdd',Now) + '.log','TDataModule1.CardPermitDelete' + aCardNo);
...
//          ADOConnectionEVENT.Connected := True;
          SQLErrorLog('DBError:('+ E.Message + ')' + SQL.Text);
          if Pos('no connection to the server',E.Message) > 0 then
          begin
            if Assigned(FOnAdoConnected) then
            begin
...
function TDataModule1.DupCheckTB_CARDFINGER(aFingerUserID,
  aPermit: string): Boolean;
var
  stSql : string;
  TempAdoQuery : TADOQuery;
