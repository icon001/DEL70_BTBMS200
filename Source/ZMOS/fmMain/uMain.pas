...
type
  ...
  private
    ...
    FAccessTypeMonitoring: Boolean; //화재전체복구 사용유무
...
//  if nSpecialProgram <> 1 then  //KT 사옥이 아니면
//  begin
      //여기서 인증 키값을 확인하자.
    stKey := GetAuthKey('2');
    if stKey = '' then
...
begin
  ...
  begin
   ...
   begin
    ...
    PeronRelayTimer1.Enabled := True;
   end else if PersonRelayType = 3 then  //경희대 로그인 하면 삼육대 서버 체크해볼것
   begin
    PeronRelayTimer1.Enabled := False;
    PeronRelayTimer1.Interval := 1000;
    PeronRelayTimer1.Enabled := True;
...
begin
  ...
  G_bRelayDefaultCode := LMDIniCtrl1.ReadString('환경설정','연동코드','001');
...
{    showmessage('지문번호 등록완료');
    stSql := ' Delete From TB_EMPHIS ';
    DataModule1.ProcessExecSQL(stSql);
    showmessage('TB_EMPHIS 삭제');
