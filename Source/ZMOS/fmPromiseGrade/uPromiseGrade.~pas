...
begin
  ...
  begin
    ...
    stDoorNO := sg_Alarm.Cells[4,AlarmLoop];
    if stNodeNo = '' then break;
    bResult := CheckTB_DEVICECARDNO_PROMISE(stPromise,stNodeNo,stECUID);
    if Not bResult then
       InsertTB_DEVICECARDNO_PROMISE(stPromise,stNodeNo,stECUID,stDoorNO,'0','L',stPermit,'ALARM')
    else UpdateTB_DEVICECARDNO_PROMISE(stPromise,stNodeNo,stECUID,stDoorNO,'0','L',stPermit,'ALARM');
...
begin
  ...
  begin
    ...
    begin
      if aDOORNO = '1' then
      begin
        if FindField('DE_DOOR1').AsString = 'Y' then result := True;
      end else
      begin
        if FindField('DE_DOOR2').AsString = 'Y' then result := True;
      end;
    end else
    begin
      if FindField('DE_USEALARM').AsString = 'Y' then result := True;
    end;
  end;
end;
