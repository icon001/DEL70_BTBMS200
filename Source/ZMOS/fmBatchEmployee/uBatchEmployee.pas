...
begin
  ...
  begin
    ...
    if stCardNo <> '' then stCardNo := FillCharString(stCardNo,'*',16);
    ...
    begin
(*aEmpID,aEmpNM,aCompanyCode,aJijumCode,aDepartCode,aPosiCode,aCompanyPhone,
                                 aJoinDate,aRetireDate,aZipcode,aAddr1,aAddr2,aHomePhone,aHandphone,
                                 afdmsID,aCardNo,aEmpImg
                                 *)
      InsertIntoTB_EMPLOYEE(stEmCode,copy(Trim(sg_Employ.Cells[2,i]),1,50),
                            stCompanyCode,stJijumCode,stDepartCode,
                            stPosiCode,copy(Trim(sg_Employ.Cells[7,i]),1,14),
                            ...
                            copy(Trim(sg_Employ.Cells[8,i]),1,14),copy(Trim(sg_Employ.Cells[9,i]),1,14),
                            stfdmsID,stCardNo,copy(Trim(sg_Employ.Cells[15,i]),1,100));
    end else
    begin
      UpdateTB_EMPLOYEE(stEmCode,copy(Trim(sg_Employ.Cells[2,i]),1,50),
                            stCompanyCode,stJijumCode,stDepartCode,
                            stPosiCode,copy(Trim(sg_Employ.Cells[7,i]),1,14),
                            stJoinDate,stRetireDate,copy(Trim(sg_Employ.Cells[10,i]),1,7),
                            copy(Trim(sg_Employ.Cells[11,i]),1,50),copy(Trim(sg_Employ.Cells[12,i]),1,50),
                            copy(Trim(sg_Employ.Cells[8,i]),1,14),copy(Trim(sg_Employ.Cells[9,i]),1,14),
                            stCardNo,copy(Trim(sg_Employ.Cells[15,i]),1,100));
(*                            stEmCode,Trim(sg_Employ.Cells[2,i]),
                            stCompanyCode,stJijumCode,stDepartCode,
                            stPosiCode,Trim(sg_Employ.Cells[7,i]),
                            stJoinDate,stRetireDate,Trim(sg_Employ.Cells[10,i]),
                            Trim(sg_Employ.Cells[11,i]),Trim(sg_Employ.Cells[12,i]),
                            Trim(sg_Employ.Cells[8,i]),Trim(sg_Employ.Cells[9,i]),
                            stCardNo,Trim(sg_Employ.Cells[15,i]));
*)
    end;
    {
...
begin
  ...
  Try
    ...
    begin
      ...
      While not Eof do
      begin
        if aCardNo <> FindField('CA_CARDNO').AsString then
        begin
          UpdateTB_DEVICECARDNODelete(FindField('CA_CARDNO').AsString,'N'); //모든 카드 권한을 삭제 처리함
        end;
        Next;
      end;
    end;
