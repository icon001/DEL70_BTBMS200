...
uses
  ...
  AdPort, OoMisc, AdPacket,WinSpool, uSubForm, CommandArray, Gauges;
...
begin
  ...
  if DBTYPE = 'MDB' then stSql := MDBSql.selectTB_ALARMDEVICEJoinPromiseCode(cmb_PromiseGrade.Text)
  else if DBTYPE = 'PG' then stSql := PostGreSql.selectTB_ALARMDEVICEJoinPromiseCode(cmb_PromiseGrade.Text)
  else if DBTYPE = 'MSSQL' then stSql := MSSQL.selectTB_ALARMDEVICEJoinPromiseCode(cmb_PromiseGrade.Text)
  else if DBTYPE = 'FB' then stSql := FireBird.selectTB_ALARMDEVICEJoinPromiseCode(cmb_PromiseGrade.Text)
  else Exit;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
            if FindField('DE_USEALARM').AsString = 'Y' then AddCheckBox(0,nRow,True,False)
