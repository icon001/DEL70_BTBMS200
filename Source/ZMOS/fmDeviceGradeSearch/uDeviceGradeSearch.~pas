...
uses
  ...
  FR_DBSet, DB, ADODB, uSubForm, CommandArray;
...
begin
  ...
  begin
    stSql := 'Select a.AL_ZONENAME as Name,a.AC_NODENO ,a.AC_ECUID as DeviceID from ';
...
begin
  ...
  if cmb_Type.ItemIndex = 0 then
    ...
    stSql := stSql + ' a.AL_ZONENAME as DEVICENAME,''방범'' as TYPE, '
  ...
  if cmb_Type.ItemIndex = 0 then
    ...
    stSql := stSql + ' Inner Join TB_ALARMDEVICE a  '
  else if cmb_Type.ItemIndex = 2 then
  ...
  begin
    if stDoorNo = '1' then //1번 출입문이면
      stSql := stSql + ' AND b.DE_DOOR1 = ''Y'' '
    else if stDoorNo = '2' then //2번 출입문이면
      stSql := stSql + ' AND b.DE_DOOR2 = ''Y'' '
    else Exit;
    if cmb_Type.ItemIndex = 0 then
...
begin
  ...
  aDoorNo := '';
  if (aGubun = 0) or (aGubun = 2) then //출입 또는 식수
  begin
