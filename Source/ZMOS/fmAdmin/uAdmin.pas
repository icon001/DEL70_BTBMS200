...
begin
  ...
  begin
    ...
    begin
      cmb_Building.ItemIndex := strtoint(Cells[8,Row]);
    end;
    if BuildingCodeList.IndexOf(cells[9,Row]) > -1 then
    begin
      cmb_AdminBuildingCode.ItemIndex := BuildingCodeList.IndexOf(cells[9,Row]);
    end;
    if cmb_AdminBuildingCode.ItemIndex > 0 then
    begin
      LoadFloorCode(cells[9,Row],cmb_AdminFloorCode);
      if FloorCodeList.IndexOf(cells[10,Row]) > -1 then
      begin
        cmb_AdminFloorCode.ItemIndex := FloorCodeList.IndexOf(cells[10,Row]);
      end;
      if cmb_AdminFloorCode.ItemIndex > 0 then
      begin
        LoadAreaCode(cells[9,Row],cells[10,Row],cmb_AdminAreaCode);
        if AreaCodeList.IndexOf(cells[11,Row]) > -1 then
        begin
          cmb_AdminAreaCode.ItemIndex := AreaCodeList.IndexOf(cells[11,Row]);
        end;
      end;
    end;
