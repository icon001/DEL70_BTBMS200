...
procedure TfmMDICardAdmin.ApdDataPacket1StringPacket(Sender: TObject;
  Data: String);
begin
...
procedure TfmMDICardAdmin.cmb_ComPortChange(Sender: TObject);
begin
...
begin
  if Not IsMaster then
  begin
    if aInsertGrade or aUpdateGrade then btn_GradeInsert.Enabled := True
    else btn_GradeInsert.Enabled := False;
    if aDeleteGrade then btn_GradeDelete.Enabled := True
