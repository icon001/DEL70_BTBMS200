...
uses
  ...
  LMDFileCtrl,ActiveX,ADODB,WinSpool, DB,IniFiles;
...
begin
  ...
  if Not SearchCardInfo(stCardNo) then
  begin
...
function TfmKTCardCreate.SearchCardInfo(aCardNo: string;aCheck:Boolean=True): Boolean;
var
  stSendData : string;
  PastTime : dword;
  stCardNo : string;
begin
  ...
  if Trim(ed_CardNo.Text) = aCardNo then Result := true
  else
  begin
    if Trim(ed_CardNo.Text) <> '' then
    begin
...
procedure TfmKTCardCreate.ApdDataPacket1StringPacket(Sender: TObject;
  Data: String);
begin
...
function TfmKTCardCreate.GetFdmsID: string;
var
  stSql : string;
  nFdms_id : integer;
begin
