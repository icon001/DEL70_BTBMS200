...
uses
  ...
  Spin, uSubForm, CommandArray, DB, ADODB,ActiveX,IMM;
...
type
  TfmDeviceCode = class(TfmASubForm)
    ...
    lb_sBuilding2: TLabel;
    lb_sFloorCode2: TLabel;
    lb_sAreaCode2: TLabel;
    Label7: TLabel;
    Label22: TLabel;
  ...
  private
    ...
    Function InsertTB_ACCESSDEVICE(aNodeNo,aMcuID,aEcuID,aMcuIp,aMcuPort,aName,aBuildingCode,
                                   ...
                                   aCurX,aCurY,aMcuAC,aMcuFd,aMcuPt,aMcuAt,aSendAck,aCardReaderType,aGubun,aUpdate,aViewSeq:string):Boolean;
    Function UpdateTB_ACCESSDEVICE(aNodeNo,aMcuID,aEcuID,aMcuIp,aMcuPort,aName,aBuildingCode,
                                   aFloorCode,aAreaCode,aRegState,aTotWidth,aTotHeight,
                                   aCurX,aCurY,aMcuAC,aMcuFd,aMcuPt,aMcuAt,aSendAck,aCardReaderType,aUpdate,aViewSeq:string):Boolean;
    Function UpdateTB_ACCESSDEVICEInfo(aNodeNo,aMcuID,aEcuID,aMcuIp,aMcuPort,aName,aBuildingCode,
                                   aFloorCode,aAreaCode,aRegState,aTotWidth,aTotHeight,
                                   aCurX,aCurY,aViewSeq:string):Boolean;
...
begin
  ...
  begin
    ...
    begin
      LoadsFloorCode(sBuildingCodeList.Strings[cmb_sBuildingCode1.ItemIndex],cmb_sFloorCode1);
      if cmb_sFloorCode1.ItemIndex > 0 then
      begin
        LoadsAreaCode(sBuildingCodeList.Strings[cmb_sBuildingCode1.ItemIndex],sFloorCodeList.Strings[cmb_sFloorCode1.ItemIndex],cmb_sAreaCode1);
      end;
    end;
  ...
  begin
    ...
    if cmb_MCUCode2.Items.Count = 2 then
    begin
      cmb_MCUCode2.ItemIndex := 1;
      stNodeNo := copy(McuCodeList.Strings[1],1,3);
    end;
    LoadsMCUCode(stBuildingCode,stFloorCode,stAreaCode,cmb_sMCUCode2);
...
begin
  ...
  begin
    ...
    if Not isdigit(cells[12,Row]) then se_McuSeq.Value := 1
    else se_McuSeq.Value := strtoint(cells[12,Row]);
    ...
    if (cells[9,Row] = '') or (cells[9,Row] = '000') then cmb_sBuildingCode1.ItemIndex := 0
      ...
      cmb_sBuildingCode1.ItemIndex := sBuildingCodeList.IndexOf(cells[9,Row]);
    stBuildingCode := sBuildingCodeList.Strings[cmb_sBuildingCode1.itemindex ];
...
begin
  ...
  begin
    ...
    LoadsBuildingCode(cmb_sBuildingCode2);
    if cmb_BuildingCode2.ItemIndex > 0 then cmb_sBuildingCode2.ItemIndex := cmb_BuildingCode2.ItemIndex;
    if cmb_sBuildingCode2.ItemIndex > 0 then
    begin
      LoadsFloorCode(sBuildingCodeList.Strings[cmb_sBuildingCode2.ItemIndex],cmb_sFloorCode2);
      if cmb_FloorCode2.ItemIndex > 0 then cmb_sFloorCode2.ItemIndex := cmb_FloorCode2.ItemIndex;
      if cmb_sFloorCode2.ItemIndex > 0 then
      begin
        LoadsAreaCode(sBuildingCodeList.Strings[cmb_sBuildingCode2.ItemIndex],sFloorCodeList.Strings[cmb_sFloorCode2.ItemIndex],cmb_sAreaCode2);
        if cmb_AreaCode2.ItemIndex > 0 then cmb_sAreaCode2.ItemIndex := cmb_AreaCode2.ItemIndex;
      end;
    end;
    ECUInsertClick;
    ed_EcuName.SetFocus;
...
function TfmDeviceCode.InsertTB_ACCESSDEVICE(aNodeNo,aMcuID, aEcuID, aMcuIp,
  ...
  aMcuAt,aSendAck,aCardReaderType,aGubun,aUpdate,aViewSeq:string): Boolean;
var
  stSql :string;
begin
  result := False;
...
function TfmDeviceCode.UpdateTB_ACCESSDEVICE(aNodeNo,aMcuID, aEcuID, aMcuIp,
  aMcuPort, aName, aBuildingCode, aFloorCode, aAreaCode, aRegState,
  aTotWidth, aTotHeight, aCurX, aCurY, aMcuAC, aMcuFd, aMcuPt,
  aMcuAt,aSendAck,aCardReaderType,aUpdate,aViewSeq:string): Boolean;
var
  stSql :string;
begin
  result := False;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          if cmb_AreaCode2.itemindex > 0 then
            stAreaCode := AreaCodeList.Strings[ cmb_AreaCode2.itemindex ];
        end;
      end;
    end;
...
begin
  ...
  begin
    ...
    if Not IsDigit(cells[11,Row]) then se_EcuSeq.Value := 1
    else se_EcuSeq.Value := strtoint(cells[11,Row]);
...
begin
    ...
    begin
      bResult := DupCheckMCU(ed_McuIp.Text,ed_McuPort.Text);
      if bResult then
      begin
          ...
          begin
            ...
            begin
              ...
              if InsertTB_ACCESSDEVICE(ed_NodeNo.Text,ed_mcuid.Text,FillZeroNumber(i,2),
                                   ...
                                   '0','0','1','0','1','0','N',inttostr(cmb_CardReaderType.ItemIndex),'1','N',inttostr(se_McuSeq.Value)) then
              begin
...
function TfmDeviceCode.UpdateTB_ACCESSDEVICEInfo(aNodeNo,aMcuID, aEcuID, aMcuIp,
  aMcuPort, aName, aBuildingCode, aFloorCode, aAreaCode, aRegState,
  aTotWidth, aTotHeight, aCurX, aCurY,aViewSeq: string): Boolean;
var
  stSql :string;
begin
  result := False;
