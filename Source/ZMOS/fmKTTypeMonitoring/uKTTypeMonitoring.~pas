...
begin
  ...
  self.FindSubForm('Main').FindCommand('KTTYPEMONITOR').Params.Values['VALUE'] := 'FALSE';
  self.FindSubForm('Main').FindCommand('KTTYPEMONITOR').Execute;
  DeviceCodeList.Free;
...
//  if stCardNo = '0000000000' then Exit;
  ...
  if Not bExitButton then
  begin
    GetUserInfo(stCardNo,stUserName,stCompanyName,stJijumName,stDepartName,stEmID);
    //카드승인유무체크
...
//  IntAlarmListView.Selected.ImageIndex := 2;
...
{  if nIndex > -1 then
  begin
    stAlarmCode := AlarmCodeList.Strings[nIndex];
    if G_nDaemonServerVersion > 1 then  //신버젼 이면
      self.FindSubForm('Main').FindCommand('SendData').Params.Values['VALUE'] := 'DEVICEALARMSTATE'+ DATADELIMITER + stAlarmCode + '0' + DATADELIMITER
    else
      self.FindSubForm('Main').FindCommand('SendData').Params.Values['VALUE'] := 'ALARMCHECK'+ DATADELIMITER + stAlarmCode + '0' + DATADELIMITER;
    self.FindSubForm('Main').FindCommand('SendData').Execute;
...
function TfmKTTypeMonitoring.GetAlarmNO(aNodeNo, aEcuID: string): integer;
var
  stSql : string;
begin
{  result := 0;
...
begin
  ...
  begin
    L_bFirst := False;
    self.FindSubForm('Main').FindCommand('KTTYPEMONITOR').Params.Values['VALUE'] := 'TRUE';
    self.FindSubForm('Main').FindCommand('KTTYPEMONITOR').Execute;
...
begin
  ...
  case aDoorStateType of
    ...
    dsLongTime : begin IntdoorListView.Items[nDoorIndex].ImageIndex:= DoorLongTimeStateImageIndex end;
...
begin
  ...
  if TDeviceState(DeviceStateList.Objects[nTempIndex]).AlarmEventState = aeAlarmEvent then Exit; //이벤트 확인하지 않은 상태면 빠져 나감
...
procedure TfmKTTypeMonitoring.AlarmEventProcess(NodeNo: integer; ECUID,
