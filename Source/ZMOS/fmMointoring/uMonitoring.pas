...
uses
  ...
  Buttons,ADODB,ActiveX, DB,iniFiles, MPlayer,uDataModule1;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          end else if TDeviceState(DeviceStateList.Objects[nDeviceIndex]).Door1State = dsClose then
          begin
        ...
        begin
          ...
          begin
            DoorList.Items[I].ImageIndex := DoorFireStateImageIndex;
          end else if TDeviceState(DeviceStateList.Objects[nDeviceIndex]).Door2State = dsClose then
          begin
            DoorList.Items[I].ImageIndex := DoorCloseStateImageIndex;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if TDeviceState(DeviceStateList.Objects[nAlarmIndex]).AlarmMode = cmArm then
        begin
          AlarmList.Items[I].ImageIndex:=AlarmModeStateImageIndex;
        end else if TDeviceState(DeviceStateList.Objects[nAlarmIndex]).AlarmMode = cmDisarm then
        begin
          AlarmList.Items[I].ImageIndex:=DisAlarmModeStateImageIndex;
        end else if TDeviceState(DeviceStateList.Objects[nAlarmIndex]).AlarmMode = cmPatrol then
        begin
          AlarmList.Items[I].ImageIndex:= AlarmPatrolStateImageIndex;
        end else if TDeviceState(DeviceStateList.Objects[nAlarmIndex]).AlarmMode = cmInit then
        begin
          AlarmList.Items[I].ImageIndex:= AlarmInitStateImageIndex;
        end else if TDeviceState(DeviceStateList.Objects[nAlarmIndex]).AlarmMode = cmTest then
        begin
...
{        if FileExists(ACAlaramFile) then
    ...
    stCompanyName,stJijumName,'',stEmID,stDeviceID,sg_Access);
    InsertDOORList(stPermitCode,stPermit,stTime,stLocationName,stDoorName,stCardNo,stUserName,
    stCompanyName,stJijumName,'',stEmID,stDeviceID,sg_IntAccess);
    InsertDOORList(stPermitCode,stPermit,stTime,stLocationName,stDoorName,stCardNo,stUserName,
    stCompanyName,stJijumName,'',stEmID,stDeviceID,sg_alarmAccess);

  end else if stMosGubun = 'FOOD' then
  begin
    if bExitButton then Exit;
    InsertDOORList(stPermitCode,stPermit,stTime,stLocationName,stDoorName,stCardNo,stUserName,
    stCompanyName,stJijumName,'',stEmID,stDeviceID,sg_Food);
  end;
...
begin
 ...
 InsertAlarmList( '2003-01-11 11:11:11', '1동-1층-1구역',
  '경계구역', '1234567890', '발생기기', 'MN', '경보발생','',0,sg_Alarm);
...
//  stMCUID := Copy(aReceiveData,8,7);
  ...
  begin
    Case cDoorMode of
      ...
      '2': //폐쇄모드
      begin
        if doorListView.Items[nIndex].StateIndex <> 5 then bRefresh := True;
        doorListView.Items[nIndex].StateIndex:= 5;
        if PatrolUse then
  ...
  case cDoorState of
    ...
    'T','D'://장시간 열림
      begin
        ...
        if doorListView.Items[nIndex].ImageIndex <> 3 then bRefresh := True;
        doorListView.Items[nIndex].ImageIndex:=3;
        if PatrolUse then
...
procedure TfmMonitoring.sg_AccessClick(Sender: TObject);
var
  stSql : string;
  stCardNo : string;
  stImage : string;
...
begin
  ...
  //begin
    ...
    fmLocationMap.L_stCreateForm := 'TfmMonitoring';
...
begin
  ...
  begin
    doorListView.LargeImages := stateShowImageList;
...
//  DeviceStateList := TStringList.Create;
  ...
  SetLength(AlarmDoorIndexArray,sg_AlarmAccess.ColCount);
  SetLength(relAlarmDoorIndexArray,sg_AlarmAccess.ColCount);
...
//  SocketWatchTimer.Enabled := True;
  ...
  sg_AlarmAccess.ColWidths[3] := 0;
  sg_AlarmAccess.ColWidths[11] := 0;
...
procedure TfmMonitoring.Panel6Resize(Sender: TObject);
begin
...
procedure TfmMonitoring.AlarmListViewDblClick(Sender: TObject);
var
  nIndex : integer;
  stAlarmID : string;
  nAlarmIndex : integer;
...
procedure TfmMonitoring.MenuItem11Click(Sender: TObject);
var
  stStatusCode : string;
  stPort : string;
  nPos : integer;
...
function TfmMonitoring.GetAlarmNO(aNodeNo, aEcuID: string): integer;
var
  stSql : string;
begin
{  result := 0;
...
procedure TfmMonitoring.DEVICEDataProcess(aNodeNo, aECUID, aData: string);
var
  cCMD :char;
  MSG_Code : char;
  stDoorNo : string;
...
procedure TfmMonitoring.DoorDataProcess(aNodeNo, aEcuID, aDoorNo, aType,
...
procedure TfmMonitoring.AlarmDataProcess(aNodeNo, aEcuID, aType, aACTION,
  aData: string);
var
  nImageIndex : integer;
  nIndex : integer;
...
procedure TfmMonitoring.ACDataProcess(aNodeNo, aECUID, aData: string);
begin
  RcvCardReadData(aNodeNo,aData);
end;
...
procedure TfmMonitoring.SearchAlarmList;
var
  stSql : string;
  nRow : integer;
...
begin
  ...
  for i:=0 to sg_AlarmAccess.ColCount - 1 do
  begin
    AlarmDoorIndexArray[i] := i;
    relAlarmDoorIndexArray[i] := i;
  end;
...
procedure TfmMonitoring.Action_AlarmHistoryExecute(Sender: TObject);
var
  stNodeNo : string;
  stEcuID : string;
  nAlarmNo : integer;
...
procedure TfmMonitoring.sg_AccessDblClick(Sender: TObject);
var
  stDoorNo : string;
  nIndex : integer;
begin
  stDoorNo := sg_Access.Cells[DoorIndexArray[11],sg_Access.Row];
...
procedure TfmMonitoring.CommandArrayCommandsTFIRERECOVERExecute(
  Command: TCommand; Params: TStringList);
var
  i : integer;
begin
...
procedure TfmMonitoring.Panel5Resize(Sender: TObject);
begin
  if doorListView.ViewStyle = vsIcon then
  begin
    doorListView.ViewStyle := vsList;
...
procedure TfmMonitoring.Panel10Resize(Sender: TObject);
begin
  if AlarmListView.ViewStyle = vsIcon then
  begin
...
procedure TfmMonitoring.N29Click(Sender: TObject);
begin
  AlarmListView.ViewStyle := vsIcon;
  AlarmListView.Refresh;
...
procedure TfmMonitoring.N30Click(Sender: TObject);
begin
  AlarmListView.ViewStyle := vsList;
  AlarmListView.Refresh;
...
procedure TfmMonitoring.pm_CloseModeClick(Sender: TObject);
var
  stDoorID : string;
  i : integer;
begin
...
procedure TfmMonitoring.AlarmModeChanged(NodeNo: integer; aEcuID: string;
...
procedure TfmMonitoring.AlarmListClear;
var
  i : integer;
begin
...
begin
  ...
  doorListView.Items[nDoorIndex].SubItems.Strings[5] := aInputType[1];
  doorListView.Items[nDoorIndex].SubItems.Strings[6] := aPermit[1];
  doorListView.Items[nDoorIndex].SubItems.Strings[7] := stEmName;
  if PatrolUse then
  begin
