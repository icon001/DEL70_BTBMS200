...
type
  ...
  private
    ...
    Function UpdateTB_EMPLOYEECode(aOrgCompanyCode,aOrgEmCode,aCompanycode,aJijumcode,aDepartcode,aPosicode,aEmpTypeCode:string):Boolean;
...
begin
  ...
  if DBTYPE = 'MSSQL' then stSql := MSSQL.SelectTB_EMPLOYEE
  else if DBType = 'PG' then stSql := PostGreSql.SelectTB_EMPLOYEE
  else if DBType = 'MDB' then stSql := MDBSql.SelectTB_EMPLOYEE
  else if DBType = 'FB' then stSql := FireBird.SelectTB_EMPLOYEE
  else Exit;
...
function TfmEmployeeBranch.UpdateTB_EMPLOYEECode(aOrgCompanyCode,
  ...
  aPosicode,aEmpTypeCode: string): Boolean;
var
  stSql : string;
  stSet : string;
begin
  ...
  stSql := CommonSql.InsertIntoTB_EMPHIS(aOrgCompanyCode,aOrgEmCode,'','2','','1',
           '','','','','','');//수정
