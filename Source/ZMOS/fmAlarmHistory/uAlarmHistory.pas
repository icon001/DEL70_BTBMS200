...
uses
  ...
  uSubForm, CommandArray, DB, ADODB;
...
begin
  Try
    ...
    if (Trim(stAC_NODENO) <> '') and (Trim(stAC_ECUID) <> '')  then
    begin
      stDeviceID := FillzeroNumber(strtoint(stAC_NODENO),3) + stAC_ECUID;
...
begin
  ...
  if DBTYPE = 'MSSQL' then
     stSql := MSSQL.SelectTB_ALARMEVENTFromDayToDay(FormatDateTime('yyyymmdd',dt_FromDate.Date),
                          FormatDateTime('yyyymmdd',dt_ToDate.Date),stNodeNo,stEcuID,stAlarmType,False)
