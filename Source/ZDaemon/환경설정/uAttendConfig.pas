...
uses
  ...
  FolderDialog, RzShellDialogs,ADODB, ExtCtrls,FileCtrl,ActiveX, RzButton,
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            ed_CardMaxCount.Text := FindField('CO_CONFIGVALUE').AsString;
          end;
...
begin
  ...
  PTAlarmUse := chk_PTAlarm.Checked;
  ...
  PTAlarmMessageUse := chk_PTMessage.Checked;
  LMDIniCtrl1.WriteString('환경설정','방범알람파일',ed_PTalarmFile.Text);
  PTAlaramFile := ed_PTalarmFile.Text;
  ...
  if cmb_CardType.ItemIndex < 0 then cmb_CardType.ItemIndex := 1;
  //if Not InsertTB_CONFIG('COMMON','CARDNOTYPE',inttostr(cmb_CardType.ItemIndex),'카드번호Type(고정/가변)') then
  //begin
  UpdateTB_CONFIG('COMMON','CARDNOTYPE',inttostr(1),'카드번호Type(고정/가변)');
  //end;

  if cmb_CardNumeric.ItemIndex < 0  then cmb_CardNumeric.ItemIndex := 1;
  if cmb_CardType.ItemIndex <> 0 then cmb_CardNumeric.ItemIndex := 1;
  //if Not InsertTB_CONFIG('COMMON','CARDNUM',inttostr(cmb_CardNumeric.ItemIndex),'카드숫자인식Type') then
  //begin
  UpdateTB_CONFIG('COMMON','CARDNUM',inttostr(1),'카드숫자인식Type');
  //end;
  ...
  //begin
  UpdateTB_CONFIG('COMMON','DUPCARD','0','1인당카드중복여부');
  //end;
  ...
  UpdateTB_CONFIG('CARD','CARDFIXED','TRUE','카드고정길이사용유무');
  UpdateTB_CONFIG('CARD','FILLCHAR','*','카드채움문자');
  //if Not rg_Front.Checked then
  //  UpdateTB_CONFIG('CARD','FILLPOSI','1','채움문자위치 0:앞,1:뒤')
  //else
  UpdateTB_CONFIG('CARD','FILLPOSI','1','채움문자위치 0:앞,1:뒤');
  UpdateTB_CONFIG('CARD','FIXEDLEN','16','카드고정길이');
  UpdateTB_CONFIG('CARD','MAXCOUNT',ed_CardMaxCount.Text,'카드최대갯수');
  ...
  Showmessage('환경설정 변경후에는 프로그램을 종료후 재시작하여 주시기 바랍니다.');
  Close;
end;
...
function TfmAttendConfig.UpdateTB_ATCODETime(aATCODE, aWorkFromTime,
...
procedure TfmAttendConfig.btn_PtAlalrmClick(Sender: TObject);
var
  st : string;
begin

  OpenDialog1.Title:= '소리 파일 찾기';
  OpenDialog1.DefaultExt:= 'wav,mp3';
  OpenDialog1.InitialDir := 'C:\WINDOWS\Media'; //C:\WINDOWS\Media
  OpenDialog1.Filter := 'WAV files (*.wav,*.mp3)|*.WAV;*.MP3';
  if OpenDialog1.Execute then
  begin
    ed_PTalarmFile.Text := OpenDialog1.FileName;
  end;

end;

procedure TfmAttendConfig.cmb_FingerTypeChange(Sender: TObject);
begin
  if cmb_FingerType.ItemIndex = 1 then
  begin
