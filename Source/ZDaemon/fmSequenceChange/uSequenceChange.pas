...
type
  ...
  private
    ...
    procedure LoadDoor(aBuildingCode,aFloorCode,aAreaCode:string;sg: TAdvStringGrid);
    procedure LoadAlarm(aBuildingCode,aFloorCode,aAreaCode:string; sg: TAdvStringGrid);
...
begin
  ...
  LoadDoor(stBuildingCode,stFloorCode,stAreaCode,sg_Door);
  LoadAlarm(stBuildingCode,stFloorCode,stAreaCode,sg_Alarm);
end;
...
begin
  ...
  if DBTYPE = 'MDB' then stSql := MDBSql.selectTB_DOORJoinAdmin(aBuildingCode,aFloorCode,aAreaCode,False)
  else if DBTYPE = 'PG' then stSql := PostGreSql.selectTB_DOORJoinAdmin(aBuildingCode,aFloorCode,aAreaCode,False)
  else if DBTYPE = 'MSSQL' then stSql := MssQl.selectTB_DOORJoinAdmin(aBuildingCode,aFloorCode,aAreaCode,False)
  else if DBTYPE = 'FB' then stSql := FireBird.selectTB_DOORJoinAdmin(aBuildingCode,aFloorCode,aAreaCode,False)
  else Exit;
