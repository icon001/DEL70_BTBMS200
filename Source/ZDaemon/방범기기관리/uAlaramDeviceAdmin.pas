...
type
  ...
  private
    ...
    procedure AlarmSearch(aNodeNo,aAlarmNo:string;aTop :integer = 0);
    procedure FormClear;
...
begin
  ...
  if cmb_regDeviceCode.ItemIndex > 0 then
  begin
    if UpperCase(State) = 'UPDATE' then AlarmSearch(stNodeNo,copy(DeviceIDList.Strings[cmb_regDeviceCode.ItemIndex],1,5),sg_Alarm.topRow)
    else AlarmSearch(stNodeNo,copy(DeviceIDList.Strings[cmb_regDeviceCode.ItemIndex],1,5));
  end else
  begin
    if UpperCase(State) = 'UPDATE' then AlarmSearch(stNodeNo,'',sg_Alarm.topRow)
