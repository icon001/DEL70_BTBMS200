...
begin
  ...
  if (ComboBox_LockType1.ItemIndex = 8) then //자바라-열림 기능을 사용하는 경우
  begin
    if Not fmMain.JavaraOpenCheck(copy(stDoorID,1,3),copy(stDoorID,4,2),copy(stDoorID,6,1)) then
    begin
      showmessage('이 컨트롤러는 자바라-열림 타입을 지원하지 않습니다. 업그레이드 후 작업하세요.');
      Exit;
    end;
  end;
  if (ComboBox_LockType1.ItemIndex = 9) then //자바라-닫힘 기능을 사용하는 경우
  begin
    if Not fmMain.JavaraCloseCheck(copy(stDoorID,1,3),copy(stDoorID,4,2),copy(stDoorID,6,1)) then
    begin
      showmessage('이 컨트롤러는 자바라-닫힘 타입을 지원하지 않습니다. 업그레이드 후 작업하세요.');
      Exit;
    end;
  end;
