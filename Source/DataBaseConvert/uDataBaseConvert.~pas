...
type
  ...
  private
    Function InsertIntoTB_ACCESSDEVICE(
        ...
        aAC_NETTYPE,aAC_COMPORT,aAC_GUBUN,aSEND_ACK,aAC_DAEMONGUBUN:string
        ):Boolean;
    ...
    Function InsertIntoTB_ALARMDEVICE(
        ...
        aAL_CURY,aLO_DONGCODE,aLO_FLOORCODE,aLO_AREACODE,aAL_OUTDELAY,aAL_INDELAY,
        aAL_ALARMID,aAL_MUXTELNO,aAL_REMOTERINGCNT,aSEND_ACK:string):Boolean;
...
begin
  ...
  begin
    ...
    begin
      ...
      if not FindField('AL_OUTDELAY').IsNull then
        nOutDelay := FindField('AL_OUTDELAY').AsInteger;
      if not FindField('AL_INDELAY').IsNull then
        nInDelay := FindField('AL_INDELAY').AsInteger;
      if not FindField('AL_REMOTERINGCNT').IsNull then
        nRemoteRingCnt := FindField('AL_REMOTERINGCNT').AsInteger;
      InsertIntoTB_ALARMDEVICE(
        ...
        inttostr(nOutDelay),
        inttostr(nInDelay),
        FindField('AL_ALARMID').AsString,
        FindField('AL_MUXTELNO').AsString,
        inttostr(nRemoteRingCnt),
        FindField('SEND_ACK').AsString
        );
      Gauge2.Progress := Gauge2.Progress + 1;
      Application.ProcessMessages;
  ...
  begin
    ...
    begin
      ...
      if not FindField('DO_CONTROLTIME').IsNull then
        nControlTime := FindField('DO_CONTROLTIME').AsInteger;
...
function TForm1.InsertIntoTB_ACCESSDEVICE(aGROUP_CODE, aAC_NODENO,
  ...
  aAC_LASTCONNECTED, aAC_NETTYPE, aAC_COMPORT, aAC_GUBUN, aSEND_ACK,
  aAC_DAEMONGUBUN: string): Boolean;
var
  stSql : string;
begin
  ...
  if Trim(aAC_DAEMONGUBUN) = '' then aAC_DAEMONGUBUN := '0';
...
function TForm1.InsertIntoTB_ALARMDEVICE(aGROUP_CODE, aAC_NODENO,
  ...
  aAL_CURY, aLO_DONGCODE, aLO_FLOORCODE, aLO_AREACODE, aAL_OUTDELAY,
  aAL_INDELAY, aAL_ALARMID, aAL_MUXTELNO, aAL_REMOTERINGCNT,
...
begin
  ...
  if Trim(aAL_OUTDELAY) = '' then aAL_OUTDELAY := '0';
  if Trim(aAL_INDELAY) = '' then aAL_INDELAY := '0';
  if Trim(aAL_REMOTERINGCNT) = '' then aAL_REMOTERINGCNT := '0';
  ...
  stSql := stSql + '''' + aLO_FLOORCODE + ''',';
  stSql := stSql + '''' + aLO_AREACODE + ''',';
  stSql := stSql + '' + aAL_OUTDELAY + ',';
  stSql := stSql + '' + aAL_INDELAY + ',';
  stSql := stSql + '''' + aAL_ALARMID + ''',';
  stSql := stSql + '''' + aAL_MUXTELNO + ''',';
  stSql := stSql + '' + aAL_REMOTERINGCNT + ',';
  stSql := stSql + '''' + aSEND_ACK + ''')';
